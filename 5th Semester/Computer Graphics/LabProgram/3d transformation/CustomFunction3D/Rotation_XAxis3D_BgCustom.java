...
public class Rotation_XAxis3D_BgCustom {
	public double[][] BGRotationX(double[][] BGOriginal, double BGAngle) {
		...
		for (int Bikram = 0; Bikram < 8; Bikram++) {
			...
			BGTransformed[Bikram][1] = Y * Math.cos(BGRadian) - Z * Math.sin(BGRadian);
			BGTransformed[Bikram][2] = Y * Math.sin(BGRadian) + Z * Math.cos(BGRadian);
		}
	...
	public void BGHehe() {
		...
		double[][] BGTransformed = BGRotationX(BGOriginal, BGAngle);
		...
		BGhelperPackage3D.createFrame("3D Rotation Along X-Axis - Custom Function With Bikram", AshliPanel);
	}

	public static void main(String[] args) {
		new Rotation_XAxis3D_BgCustom().BGHehe();
	}
}